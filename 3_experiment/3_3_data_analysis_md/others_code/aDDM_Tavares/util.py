...
def get_empirical_distributions(data, timeStep=10, maxFixTime=10000,
                                numFixDists=3, fixDistType="simple",
                                valueDiffs=range(-3,4,1), subjectIds=None,
                                useOddTrials=True, useEvenTrials=False,
                                useCisTrials=False, useTransTrials=False):
    """
...
def generate_choice_curves(dataTrials, simulTrials, pdfPages):
    ...
    countTotal = np.zeros(len(unique_summed_vals)) 
    countLeftChosen = np.zeros(len(unique_summed_vals))
    ...
    for trial in dataTrials:
        ...
        idx = unique_summed_vals.index(valueDiff)
    ...
    stdProbLeftChosen = np.zeros(len(unique_summed_vals))
    probLeftChosen = np.zeros(len(unique_summed_vals))
    for i in xrange(0,len(unique_summed_vals)):
    ...
    fig = plt.figure()
    plt.errorbar(unique_summed_vals, probLeftChosen, yerr=stdProbLeftChosen,
...
def generate_rt_curves(dataTrials, simulTrials, pdfPages):
    ...
    for valueDiff in unique_summed_vals:
    ...
    meanRTs = np.zeros(len(unique_summed_vals))
    stdRTs = np.zeros(len(unique_summed_vals))
    for valueDiff in unique_summed_vals:
        idx = unique_summed_vals.index(valueDiff)
    ...
    colors = cm.rainbow(np.linspace(0, 1, 9))
    fig = plt.figure()
    plt.errorbar(unique_summed_vals, meanRTs, yerr=stdRTs, label="Data",
                 color=colors[0])
    ...
    for trial in simulTrials:
        valueDiff = trial.valueLeft + trial.valueRight  # changed subtractino to ADDITION
        RTsPerValueDiff[valueDiff].append(trial.RT)
    ...
    plt.errorbar(unique_summed_vals, meanRTs, yerr=stdRTs, label="Simulations",
                 color=colors[5])
    plt.xlabel("Value difference")
    plt.ylabel("Mean RT")
    plt.legend()
