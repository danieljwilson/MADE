function NLLData = NLLSimData_fromValswBias(Data, simParams, ProbMat, vals, paramIDs)
...
dataVals = Data.attribVals * simParams(1:2)';  % 1 and 2 are weight on face/house

Data.rt = Data.rt - simParams(5); % correct RT for non-decision time (subtracted)
...
for i = 1:length(Data.resp)
    ...
    % recode starting bias for negative value simulations (since value
    % simulations were only done with positive values)
    if dataVals(i) < 0
        simParams(6) = -1*simParams(6);  % this is flipping the starting bias
    end
    
    ProbMatToUse = ProbMat(Driftbin).SimData.ProbMat{paramIDs.upper == simParams(3), ...
                                                     paramIDs.collapseRate == simParams(4), ...
                                                     paramIDs.startBias == simParams(6)};
    ...
    RTbin = round(Data.rt(i) * 1000);   % convert ms rt (observed RT!)
    ...
    if ~isnan(RTbin) % 
        ...
        pNonResp = ProbMatToUse(end,1) ...    %simParams(5) = nonDT
            + sum(temp1((maxRT*1000 - simParams(5)*1000 + 1):end)) ...  % temp1 = Yes
            + sum(temp2((maxRT*1000 - simParams(5)*1000 + 1):end));     % temp2 = No
