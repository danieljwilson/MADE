...
mkdir(fullfile(homepath, 'ATP1','Analysis','FittedResults',model))
...
combo.weight1 = [-.01:.01:.03];
combo.weight2 = [-.01:.01:.03];
combo.weight3 = [-.01:.01:.03];
combo.upper = (4:4:20)/100;
combo.collapseRate = 0:.0002:.0008;
combo.nonDec = 0:.15:.6;
...
for c = 1:2
    ...
    tempData.attribVals = [Data.SelfProposal(selectedTrials)' - 10 ...
                           ...
                           -1 * abs(Data.SelfProposal(selectedTrials)' - Data.OtherProposal(selectedTrials)')];
    
    
    uniqueVals = unique(abs(tempData.attribVals*parameterCombos(:,1:3)'));
    ...
    ddmFile = fullfile(path,'Analysis','FittedResults',model, ['DDM_fittedResults_m' model '_' subject '_' conds{c} '.mat']);
    ...
    if any(SimNLL(:) == 0) % run simulations if not complete yet
        ...
        for loop = 1:nLoops
            ...
            if SimNLL(loop) == 0
                ...
                % parameter values
                SimNLL(loop) = NLLSimData4_fromValswBias(tempData,parameterCombos(loop,:),valStruct,uniqueVals, simInfo.combo);
    ...
    params.inputvals = repmat(tempData.attribVals*parameterCombos(i,1:3)',1000,1);
    params.upper = BestParameters(4);
    params.lower = -1*BestParameters(4);
    params.collapseRate = BestParameters(5);
    params.startBias = BestParameters(7);
    %params.nonDec = BestParameters(5);
      
    SimData = simulDDM(params);
    SimData.rt = SimData.rt + BestParameters(6);
    ...
    params.candidate_driftSelf = combo.weight1;
    params.candidate_driftOther = combo.weight2;
    params.candidate_driftFairness = combo.weight3;
