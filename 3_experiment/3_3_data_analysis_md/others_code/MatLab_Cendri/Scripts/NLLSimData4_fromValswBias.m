function NLLData = NLLSimData4_fromValswBias(Data, simParams, ProbMat, vals, paramIDs)
...
dataVals = Data.attribVals * simParams(1:3)'; 
Data.rt = Data.rt - simParams(6); % correct RT for non-decision time
...
for i = 1:length(Data.resp)
    ...
    if dataVals(i) < 0
        simParams(7) = -1*simParams(7);
    end
    
    ProbMatToUse = ProbMat(Driftbin).SimData.ProbMat{paramIDs.upper == simParams(4), ...
                                                     paramIDs.collapseRate == simParams(5), ...
                                                     paramIDs.startBias == simParams(7)};
    ...
    if ~isnan(RTbin) % 
        ...
        pNonResp = ProbMatToUse(end,1) ...
            + sum(temp1((maxRT*1000 - simParams(6)*1000 + 1):end)) ...
            + sum(temp2((maxRT*1000 - simParams(6)*1000 + 1):end));
