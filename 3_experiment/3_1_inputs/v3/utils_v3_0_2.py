...
def task_trials(win, expInfo, Rand_Stimuli, practice_trial_num,
    ...
    dataFile = open(fileName + '.csv', 'w')  # a simple text file with 'comma-separated-values'
    dataFile.write(
        'date, psychopy_version, exp_version, face_version, house_version, left, subject, block, trial, rt, response, correct, summed_val_total, face_image, face_val_base, face_mult, face_val_total, house_image, house_val_base, house_mult, house_val_total, fix_num, fix_rt, fix_stim, accuracy\n')
    ...
    for block in range(blocks):
        ...
        for trial in trial_count[block]:
            ...
            if trial_type == 'task':
                ...
                visual.Rect(win, units='norm', width=0.3, height=0.25, pos=[-0.85,-0.625], color=(-0.5, -.2, -.2)).draw()
                visual.TextStim(win, units='norm', pos=[-0.85,-0.625], height=0.16, color=(1), text='$2').draw()
                visual.Rect(win, units='norm', width=0.3, height=0.25, pos=[-0.85,-0.375], color=(0, -.2, -.2)).draw()
                ...
                visual.Rect(win, units='norm', width=0.3, height=0.25, pos=[-0.85, 0.125], color=(0.35, -.2, -.2)).draw()
                visual.TextStim(win, units='norm', pos=[-0.85, 0.125], height=0.16, color=(1), text='$14').draw()
                visual.Rect(win, units='norm', width=0.3, height=0.25, pos=[-0.85, 0.375], color=(0.5, -.2, -.2)).draw()
                visual.TextStim(win, units='norm', pos=[-0.85, 0.375], height=0.16, color=(1), text='$20').draw()
                visual.Rect(win, units='norm', width=0.3, height=0.25, pos=[-0.85, 0.625], color=(0.65, -.2, -.2)).draw()
            ...
            ### to the next "level")
                ...
                # report accuracy
                visual.Rect(win, units='norm', pos=[0,-0.5], width=0.9 , height=0.17,
                            color=-1).draw()        # background rect
                visual.TextStim(win, units='norm', pos=[0, -0.5], height=0.14, color=(1),
            ...
            if trial_type == 'task':
                for fix_num in range(len(fix_array)):
                    dataFile.write(
                        '%s,%s,%s,%i,%i,%s,%i,%i,%i,%.3f,%i,%i,%.2f,%s,%.2f,%.2f,%.2f,%s,%.2f,%.2f,%.2f,%i,%.3f,%s,%.2f,%i\n' % (expInfo['dateStr'],
                                                                                                                                expInfo['psychopy_version'],
                                                                                                                                expInfo['exp_version'],
                                                                                                                                expInfo['face_version'],
                                                                                                                                expInfo['house_version'],
