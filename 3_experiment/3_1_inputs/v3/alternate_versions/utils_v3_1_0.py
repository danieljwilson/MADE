...
def instructions_3(win, task_trial_num, blocks):
    ...
    # trials1
    visual.TextStim(win, pos=[0, 0], height=0.9,
                    text='You will start with just 1 block of trials, with {} trials in this block.\n\nAfter you are done this block you will receive some additional instructions for the next blocks.'.format(int(task_trial_num/blocks))).draw()
    show(win)
    # start1
...
def instructions_4(win, Stimuli, tp_rt):
    """
    ...
    # explain_combo1
    visual.TextStim(win, pos=[0, 0], height=0.9,
                    text='Nice work.\n\nWe are now going to change things a little bit.\n\nFor the remaining trials you will have a limited amount of time to make your choice.').draw()
    show(win)
    # explain_combo2
    visual.TextStim(win, pos=[0, 0], height=0.9,
...
def task_tp_trials(win, expInfo, Rand_Stimuli, practice_tp_trial_num,
                task_trial_num, tp_rt, blocks, trial_type):
    """
    Function that shows two stimuli and requires subjects to either 'accept' or
    'reject'. Feedback is provided after the choice is made.
    ...
    dataFile = open(fileName + '.csv', 'w')  # a simple text file with 'comma-separated-values'
    dataFile.write(
        'date, psychopy_version, exp_version, face_version, house_version, left, subject, block, trial, rt, response, correct, summed_val_total, face_image, face_val_base, face_mult, face_val_total, house_image, house_val_base, house_mult, house_val_total, fix_num, fix_rt, fix_stim, fix_num_total, accuracy, tp_rt, payout\n')
    ...
    if trial_type == 'practice':
        ...
        block_range = range(1,blocks)

    # iterate through blocks, but starting with the SECOND block
    for block in block_range:
        ...
        for trial in trial_count[block]:
            ...
            if trial_type == 'practice':
                for fix_num in range(len(fix_array)):
                    dataFile.write(
                        '%s,%s,%s,%i,%i,%s,%i,%i,%i,%.3f,%i,%i,%.2f,%s,%.2f,%.2f,%.2f,%s,%.2f,%.2f,%.2f,%i,%.3f,%s,%i,%.2f,%.2f,%i\n' % (expInfo['dateStr'],
                                                                                                                                    expInfo['psychopy_version'],
                                                                                                                                    expInfo['exp_version'],
                                                                                                                                    expInfo['face_version'],
                                                                                                                                    expInfo['house_version'],
    ...
    if trial_type == 'task':
        ...
        # complete
        visual.TextStim(win, pos=[0, 0], height=0.9,
                        text='Well done, you have completed the main task!\n\nYou earned a total of ${:.2f}.\n\nSince this was 3 out of 4 blocks, you earned 3/4 * ${:.2f} = ${:.2f}'.format(payout, payout, payout*0.75)).draw()
        show(win)
    dataFile.close()
    return(accuracy)
...
def instructions_5(win, task_trial_num, tp_rt, blocks=1):
    ...
    # start_task
    visual.TextStim(win, pos=[0, 0], height=1.5,
                    text='You will now begin the time pressure trials.').draw()
    show(win)
    # winning_money1
    visual.TextStim(win, pos=[0, 0], height=0.9,
    ...
    # trials1
    visual.TextStim(win, pos=[0, 0], height=0.9,
                    text='There will be {} more blocks of trials, with {} trials in each block.\n\nYou will have the chance to take a short break after each block.'.format(blocks-1, int(task_trial_num/blocks))).draw()
    show(win)
    # start1
    visual.TextStim(win, pos=[0, 0], height=1.2,
                    text='When you are ready to START press any key to begin...').draw()
