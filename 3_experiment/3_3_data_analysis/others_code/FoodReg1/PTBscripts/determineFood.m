...
for i = 1:length(ChoiceData.FoodOnTrial)
   if ChoiceData.Choice{i} == -1
       availableTrials = [availableTrials i];
   else
       if ~isempty(intersect(ChoiceData.FoodOnTrial(i), availableFoods)) && ChoiceData.Choice{i} == 1
           availableTrials = [availableTrials i];
       end
   end
end
...
showInstruction(38,PTBParams,'RequiredKeys',{'RightArrow','right'});
...
[FoodPic, FoodPicSize] = makeTxtrFromImg(fullfile(PTBParams.foodPath,Food),...
    'JPG',PTBParams);
FoodPicPosition = findPicLoc(FoodPicSize, [.5, .6], PTBParams, 'ScreenPct', .4); %%%%% I use the FoodPic variable from line 97 right?
...
DrawFormattedText(PTBParams.win,[foodText],...
    'center',1.75*PTBParams.ctr(2),PTBParams.white,55,[],[],1.75);
