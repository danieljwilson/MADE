...
def main():
    ...
    a = np.unique(expdata.item_left_val)
    b = np.unique(expdata.item_right_val)
    ...
    # Remove line 147: instead of simulation/condition use TOTAL simulations [before simulate, sample from value left and value right np.random.choice to take value from arrays]
    for (valueLeft, valueRight) in trialConditions:
        for s in xrange(args.simulations_per_condition):
            #Set value left and value right here
            try:
                simulTrials.append(
