function [SimNLL, BestParameters] = fitDDMFromSim_testFit(subject)
...
combo.weight1 = [-.02:.005:.04, -.002, .002];
combo.weight2 = [-.02:.005:.04, -.002, .002];
combo.upper = (4:4:32)/100;
combo.collapseRate = [0, .0005, .001, .003, .005, .007, .01];
combo.nonDec = 0:.1:.6;
...
% simulate a test subject's parameters
...
params.precision = .01;
params.s = .1;
params.inputvals = repmat(valuepairs*[.01, .005]',1000,1);
params.upper = .08;
params.lower = -1*params.upper;
params.collapseRate = .001;
    %params.nonDec = BestParameters(5);
      
SimData = simulDDM(params);
SimData.rt = SimData.rt + .3;
